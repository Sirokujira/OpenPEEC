...
**変更したら必ず `peec_check.sh` を通すこと。** 40 件すべて OK でなければ
...
## 既知の制限 (実装しようとして壁に当たったものを含む)

- 体積導体・厚み方向の電流分布は未対応。面導体は厚さ 1 層。
  **`plate` に厚さ方向分割を足す実装を試みたが動作しなかった**: 面内セルが
  厚さゼロのリボン (面積分)、層間の接続が細線 (等価半径近似) となり、
  積層構造でインダクタンス行列が使える形にならず解が NaN になる。
  対応するには部分要素カーネルを体積定式化 (平行矩形バーは Hoer–Love の
  閉形式、非平行は 3 次元求積) に置き換える必要がある。
