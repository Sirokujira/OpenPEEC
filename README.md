...
- 面導体は平面矩形のみ (曲面・非矩形は矩形の集合で近似)。厚さ方向は
  既定で 1 層で、その場合の表皮効果は角線と同じ合成式を面セルの断面
  (幅 × 厚さ) に適用します。厚み方向の電流分布を解くには `分割数t` ≥ 2
  (体積セル) を使ってください (層厚 ≲ δ/2 で表皮効果を分解できます)。
  体積セルの閉形式は軸・断面がそろった平行バー対に適用し、それ以外の
  対 (斜め配置など) は中立面リボンの面積分に落ちます。
